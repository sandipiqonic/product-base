@props(['userName','userComment','userAvatar','postImg','image','addBtn'])
<div class="d-flex justify-content-center gap-3">
    <img src="{{asset('modules/social-app/images')}}/{{$userAvatar}}" class="img-fluid avatar-48 rounded-circle" alt="profile-image-1">
    <div class="d-flex align-items-center justify-content-between flex-grow-1">
        <div class="me-3">
            <h5>{{$userName}}</h5>
            <small class="text-ellipsis short-1">{{$userComment}}</small>
        </div>
        @if ($image ?? '')
            <img src="{{asset('modules/social-app/images')}}/{{$postImg}}" class="img-fluid avatar-40 rounded object-contain" alt="profile-image">
        @endif
        @if ($addBtn ?? '')
            <div>
                <button class="btn btn-sm btn-icon btn-primary rounded-pill">
